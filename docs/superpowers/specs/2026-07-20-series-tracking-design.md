...
2. **One-time backfill**, run once against every existing `Book` row whose `title` matches the pattern and whose `seriesName` is currently `null`. Confirmed precedent in this repo (`prisma/migrations/20260716192026_unify_copy_types/migration.sql`): a one-time data backfill is done as raw SQL embedded directly in the same Prisma migration that adds the new columns, not a standalone script (there is no `scripts/` directory in this repo). The regex isn't expressible as plain SQL, so this migration's data step instead runs a small companion TypeScript function (invoked once, e.g. from a `postinstall`-adjacent one-off command or directly via `prisma db execute`/a raw query run manually against each environment after the migration lands) that reads every `Book` row, applies `parseSeriesFromTitle`, and updates matches — this is a one-time operational step, not new application code that ships permanently. There is nothing to "re-sync" after that first parse, since `Book.title` never changes after creation in this app (confirmed: no code path ever updates an existing book's `title` from a Goodreads re-sync — only the initial creation writes it).
...
`/books/[id]/edit` gains two new fields, alongside the existing title/author/isbn fields in `EditBookForm`:
...
- **No re-parsing on an ongoing basis** — parsing happens once, at creation (or once, at backfill, for pre-existing rows). Since `Book.title` never changes post-creation in this app, there is no "keep this in sync with Goodreads" concern the way read status/rating have.
