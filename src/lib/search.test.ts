...
afterEach(async () => {
  // `contains` (not `startsWith`): other files sharing this fixture-cleanup
  // pattern (search.startsWith.test.ts) put a fixture title's target letter
  // as the literal first character, with "Test Search" elsewhere in the
  // string rather than at the very start -- `startsWith` would silently
  // fail to clean those up. Kept identical here for consistency even though
  // this file's own fixtures don't need it.
